...
//		}
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE,$userT=FALSE)
	{
		...
		$this->trigger_events('extra_where');
		$extraSelect = "mobile";
		// $query = $this->db->select($this->identity_column . ', id, name, password,last_login, is_active')
		//                   ->where($this->identity_column, $this->db->escape_str($identity))
		//                   ->limit(1)
		//                   ->get($this->tables['users']);
		$query = $this->db->select($this->identity_column .", ".$extraSelect.',name, mobile, email, id, password,is_active, last_login')
							->where($this->identity_column, $identity)
		...
		if ($query->num_rows() === 1)
		{
			...
			//$this->db->_compile_select(); 
				$aaa1= $this->get_users_groups($user->id)->result(); 
				if($userT=='admin'){
					if($aaa1[0]->id!=1){
			   ...
			   // $this->set_session("user_group",$aaa1);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		//if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');
		return $this->db->query("SELECT groups.id,groups.name,groups.description FROM groups JOIN users_groups ON groups.id=users_groups.group_id JOIN users ON users.id=users_groups.user_id WHERE users.id=$id");
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		//print_r(array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id)); die;
