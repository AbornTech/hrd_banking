...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
            ...
            $user = $this->where($this->config->item('identity', 'ion_auth'), $identity)->where('is_active', 1)->users()->row();  //changed to get_user_by_identity from email
             $controller = 'front';
			if ($user)
			{
	...
	public function is_admin($id=false)
	{
		...
		$admin_group = $this->config->item('groups', 'ion_auth');
		$aa = $this->in_group($admin_group, $id); 
		return $aa;
	}
